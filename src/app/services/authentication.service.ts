...
export class AuthenticationService {
  ...
  login(credentials: { email: string, password: string }): Observable<any> {
    return this.http.post<any>(environment.apiUrl + '/auth/login', credentials, { observe: 'response', responseType: 'json', withCredentials: true })
      .pipe(
        tap(response => {
  ...
  logout(): Observable<any> {
    ...
    return this.http.get(environment.apiUrl + '/auth/logout', { responseType: 'text', withCredentials: true });
  }
  ...
  //   return this.http.get('http://localhost:5000' + '/users/' + userId + '/avatar',
  //     { responseType: 'text', withCredentials: true });
  // }
  getUserAvatar(userId: string): Observable<any> {

    return this.http.get(environment.apiUrl + '/users/' + userId + '/avatar', { responseType: 'text', withCredentials: true });
  }
  ...
  updateUserAvatar(avatar: File, userId: string): Observable<any> {
    ...
    return this.http.put(environment.apiUrl + '/users/' + userId + '/updateAvatar', formData, {
      responseType: 'text',
  ...
  insertAvatar(avatar: string, userId: string): Observable<any> {
    return this.http.put(environment.apiUrl + '/users/' + userId + '/avatar', { avatar }, { responseType: 'text', withCredentials: true });
  }
  ...
  uploadUserAvatar(userId: string, avatar: File): Observable<any> {
    return this.http.post(environment.apiUrl + '/users/' + userId + '/avatar', avatar);
  }
